...
export class Parser {
    ...
    public typeHandlers: Record<string, handler[]> = {};
    public listContextStack: any[] = [];
    ...
    _processTokens(context: Context) {
    ...
    _manageListContext(token: Token, index: number, context: Context) {
        const isListItem = token.type === TokenType.UNORDERED_LIST_ITEM ||
            token.type === TokenType.ORDERED_LIST_ITEM;
        const isOrderedList = token.type === TokenType.ORDERED_LIST_ITEM;


        if (isListItem) {
            const prevToken = index > 0 ? this.tokens[index - 1] : null;
            const isPrevSameListType = prevToken &&
                ((isOrderedList && prevToken.type === TokenType.ORDERED_LIST_ITEM) ||
                    (!isOrderedList && prevToken.type === TokenType.UNORDERED_LIST_ITEM));
            ...
            if (!isPrevSameListType) {
                ...
                listStartHandlers.forEach(handler => {
                    handler({ type: TokenType.LIST_START, listType: listContext.type }, context);
                });
            }
            ...
            const isNextSameListType = nextToken &&
                ((context.currentListContext.type === 'ol' && nextToken.type === TokenType.ORDERED_LIST_ITEM) ||
                    (context.currentListContext.type === 'ul' && nextToken.type === TokenType.UNORDERED_LIST_ITEM));
